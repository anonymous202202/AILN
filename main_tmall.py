...
from model_tmall import AILNModel
import pandas as pd
from data_loader import UnidirectTrainDataset, UnidirectTestDataset, UnidirectValidateDataset
import metric

here = os.path.dirname(os.path.abspath(__file__))
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
parser = argparse.ArgumentParser()
parser.add_argument('--tvt_path', default='IJCAI/tvt_alin.pkl')
parser.add_argument('--cate_num', type=int, default=729, help='item number')  # 总共729个类别
parser.add_argument('--item_num', type=int, default=14601, help='item number')  # 总共14601个物品
...
parser.add_argument('--epoch', type=int, default=500, help='the number of epochs to train for')
parser.add_argument('--lr', type=float, default=0.001, help='learning rate')
parser.add_argument('--lamda', type=float, default=1, help='reg')
parser.add_argument('--testloader_file', default='IJCAI/test_ailn.pkl')
parser.add_argument('--trainloader_file', default='IJCAI/train_ailn.pkl')
parser.add_argument('--valloader_file', default='IJCAI/val_ailn.pkl')
...
def get_itemcate():
    itemcate = {0: 0}
    data = pd.read_csv('IJCAI/reid_data_fin.csv', names=["user_id", 'item_id', 'cate_id', 'time_stamp'], dtype=int,
...
def main():
    print('Loading data...')
    if os.path.exists(args.trainloader_file) and os.path.exists(args.testloader_file) and os.path.exists(
            args.valloader_file):
...
def trainForEpoch(train_instance, train_loader, model, optimizer, criterion):
    ...
    for i, data in enumerate(train_loader):
        ...
        if ((i + 1) % 128) == 0 or (i + 1) == train_instance:
            ...
            batch_loss += args.lamda * l2_regularization
            batch_loss.backward()
            optimizer.step()
            batch_loss = torch.tensor(0).float().to(device)
